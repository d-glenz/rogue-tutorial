...
fn handle_keys(tcod: &mut Tcod, game: &mut Game, objects: &mut Vec<Object>) -> PlayerAction {
	...
	Pick up item: g
	Show inventory: i
        Show character information: c
	Drop equipped item: d
	Climb stairs: <");
            msgbox(&msg, CHARACTER_SCREEN_WIDTH, &mut tcod.root);
            DidntTakeTurn
        }
...
fn new_game(tcod: &mut Tcod) -> (Game, Vec<Object>) {
    ...
    game.messages.add(
        "To move around, use your arrow keys (or the numpad for more flexibility).",
        WHITE,
    );
    game.messages.add(
        concat!(
            "To pick up an item, press 'g'. To see your inventory, press 'i'. ",
            "To attack orcs (o) or trolls (T), move in their direction."
        ),
        WHITE,
    );
...
fn main_menu(tcod: &mut Tcod) {
    let img = tcod::image::Image::from_file("resources/menu_background.png")
        .ok()
